try {
  ...
  async function getWebsiteName() {
    ...
    if (websiteName.startsWith('www.')) {
      websiteName = websiteName.slice(4); // Remove 'www.' if present
    }
  ...
  chrome.runtime.onInstalled.addListener(() => {
    chrome.contextMenus.create({
      ...
      title: 'Summarize Privacy Policy and Terms of Service',
      contexts: ['all']
    });
  });
  ...
  //       console.log('Policy Content:', html);
  //       return html;
  //     } catch (error) {
  ...
  async function summarizePolicy(policyContent, policyType) {
    ...
    // hyrIffr9kU7d6DEHL4LOT3BlbkFJdvcjv7y7AaBT0Tje2x51
    try {
